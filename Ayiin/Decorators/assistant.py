...
from Ayiin import BOT_ID, MUSIC_BOT_NAME, app, random_assistant
...
async def unban_assistant_(_, CallbackQuery):
    ...
    if not a.can_restrict_members:
        return await CallbackQuery.answer(
            "Saya tidak memiliki izin pengguna untuk melarang/membatalkan larangan. Minta admin mana pun untuk unban asisten.",
            show_alert=True,
        )
    else:
        try:
        ...
        try:
            ...
            if b.status == "kicked":
                return await message.reply_text(
                    f"Akun Asisten[{ASS_ID}] di ban.\nBatalkan pemblokirannya (Unban) terlebih dahulu untuk menggunakan Bot Musik\n\nUsername: @{ASS_USERNAME}",
                    reply_markup=key,
                )
            if b.status == "banned":
                return await message.reply_text(
