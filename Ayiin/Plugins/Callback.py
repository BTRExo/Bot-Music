...
from Ayiin import BOT_USERNAME, MUSIC_BOT_NAME, app, db_mem
from Ayiin.Core.PyTgCalls import Queues
from Ayiin.Core.PyTgCalls.Converter import convert
from Ayiin.Core.PyTgCalls.Downloader import download
from Ayiin.Core.PyTgCalls.Ayiin import (join_stream, pause_stream,
...
from Ayiin.Database.queue import (add_active_chat, is_active_chat,
                                  is_music_playing, music_off, music_on,
                                  remove_active_chat)
from Ayiin.Decorators.admins import AdminRightsCheckCB
from Ayiin.Decorators.checker import checkerCB
from Ayiin.Inline import (audio_markup, audio_markup2, download_markup,
                          fetch_playlist, paste_queue_markup, primary_markup,
                          secondary_markup2)
from Ayiin.Utilities.changers import time_to_seconds
...
from Ayiin.Utilities.timer import start_timer
...
async def play_playlist(_, CallbackQuery):
    ...
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Pemutaran Streaming berlangsung... Hentikan untuk memutar daftar putar",
                show_alert=True,
            )
        else:
            pass
    ...
    if not _playlist:
        ...
        for_t = 0
        for_p = 0
        for yins in _playlist:
            _note = await get_playlist(user_id, yins, type)
            title = _note["title"]
            ...
            if await is_active_chat(chat_id):
                ...
                if not await join_stream(chat_id, raw_path):
                    return await mystic.edit(
                        "Error tidak dapat Bergabung dengan Obrolan Suara. Pastikan Obrolan Suara Diaktifkan."
                    )
                theme = await check_theme(chat_id)
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"No {genre} Daftar putar di server. Coba tambahkan musik di daftar putar.",
            show_alert=True,
        )
    else:
        j = 0
        ...
        for yins in _playlist:
            j += 1
            _note = await get_playlist(user_id, yins, genre)
            title = _note["title"]
            duration = _note["duration"]
            msg += f"{j}- {title[:60]}\n"
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        ...
        await CallbackQuery.answer()
        for yins in _playlist:
            await delete_playlist(user_id, yins, genre)
    await CallbackQuery.message.reply_text(
