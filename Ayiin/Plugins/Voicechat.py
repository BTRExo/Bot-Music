...
from Ayiin import SUDOERS, app, db_mem, random_assistant
...
/joinassistant [Username Obrolan atau ID Obrolan]
- Bergabunglah dengan asisten ke grup.


/leaveassistant [Username Obrolan atau ID Obrolan]
- Asisten akan meninggalkan grup tertentu.


/leavebot [Username Obrolan atau ID Obrolan]
...
async def basffy(_, message):
    ...
    if not _assistant:
        return await message.reply_text(
            "Asisten yang Disimpan Sebelumnya Tidak Ditemukan.\n\nAnda dapat menyetel Asisten Melalui /play di dalam Grup {Chat}"
        )
    else:
...
async def baujaf(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**Usage:**\n/leave [Username Obrolan atau ID Obrolan]
        )
        return
    chat = message.text.split(None, 2)[1]
    try:
